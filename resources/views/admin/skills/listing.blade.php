        ...
        <div class="custom-table-responsive">
            <table class="table table-sm  ">
                <thead class="thead-blue">
                    <th width="30"><input type="checkbox" style="height: 16px; width: 16px;" class="bulk-action all">
                    </th>
                ...
                <tbody>
                    @if (isset($skills) && $skills->count() > 0)
                        @foreach ($skills as $skill)
                            <tr>
                                <td><input value="{{ $skill->id }}" class="bulk-action single" type="checkbox"
                                ...
                                <td class="text-capitalize">
                                    <div class="d-flex">
                                        <span class="capsule-shape capsule-shape-{{ $skill->id }}"
                                            style="top:-1px;background-color: {{ $skill->background_color ?? 'transparent' }}; color : {{ $skill->text_color ?? 'current' }} ">
                                            {{ $skill->skill ?? '' }}
                                        </span>
                                        <div class="d-flex ml-5">
                                            <div data-set-color="{{ !empty($skill->background_color) ? $skill->background_color : 'rgb(0, 0 ,0)' }}"
                                                data-target="{{ '.capsule-shape-' . $skill->id }}"
                                                class="bcPicker mr-2" data-toggle="tooltip" title="set background"
                                                style="height: 30px; width:30px;top:0px;"></div>
                                            <div data-set-color="{{ !empty($skill->text_color) ? $skill->text_color : 'rgb(0, 0 ,0)' }}"
                                                data-target="{{ '.capsule-shape-' . $skill->id }}" class="bcPicker2"
                                                data-toggle="tooltip" title="set text color"
                                                style="height: 30px; width:30px;top:0px;"></div>
                                        </div>
                                    </div>
                                ...
                                <td>
                                    <div class="btn-group" data-id="{{ $skill->id }}">
                                        ...
                                        <a href="Javascript:void(0);"
                                            data-action="{{ route('admin.skills.destroy', $skill->id) }}"
